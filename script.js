...
async function handleTTS() {
    const textToSpeak = "Merry Christmas Bindhu Chechi! Wishing you endless joy and happiness this festive season. Your thoughtful message is now spoken!";
    ...
    ttsStatus.textContent = 'Failed to generate audio after several retries.';
    ttsBtn.disabled = false;
}
...
async function generateRecipe() {
    ...
    while (currentRetry < maxRetries) {
        try {
            ...
            if (currentRetry < maxRetries) {
                const delay = Math.pow(2, currentRetry) * 1000;
                recipeOutput.textContent = `Error generating recipe. Retrying in ${delay / 1000}s...`;
                await new Promise(resolve => setTimeout(resolve, delay));
            }
        }
    }
    recipeOutput.textContent = 'Failed to generate recipe after several retries. Maybe the API server is having a snow day!';
    generateRecipeBtn.disabled = false;
    recipeLoader.style.display = 'none';
}
